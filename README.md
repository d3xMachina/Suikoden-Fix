...
- After saving in-game, go to `(GAME_PATH)\SuikodenFix\Decrypted\gsd1\` for Suikoden 1 or `(GAME_PATH)\SuikodenFix\Decrypted\gsd2\` for Suikoden 2. You will have the save file in the json format. The file name starts with `Data` followed by the save slot number.
